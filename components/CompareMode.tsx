...
export default function CompareMode({
  ...
  const selectedNames = selected.map(
    (slug) => resorts.find((r) => r.slug === slug)?.name ?? slug
  )
      ...
      {active ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
          {resorts.map((resort, index) => {
            const isSelected = selected.includes(resort.slug)
            const isDisabled = selected.length >= 2 && !isSelected
            return (
              <div
                key={resort.slug}
                className="relative cursor-pointer"
                onClick={() => toggleSelect(resort.slug)}
              >
                {/* Selection overlay */}
                <div
                ...
                {/* Render the actual card - pointer-events-none to let parent handle click */}
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-lg">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              {match ? (
                <button
                  onClick={() => router.push(`/ski/compare/${match.slug}`)}
                  className="px-5 py-2 bg-sky-600 text-white rounded-full text-sm font-medium hover:bg-sky-700 transition-colors"
                >
