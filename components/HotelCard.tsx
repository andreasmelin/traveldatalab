...
export default function HotelCard({
  ...
  const badgeActiveClass = isParks
    ? 'bg-emerald-100 text-emerald-700 font-semibold'
    : 'bg-sky-100 text-sky-700 font-semibold'
  const ctaClass = isParks
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mb-2">
            <span className={`text-xs px-2 py-0.5 rounded-full ${hotel.skiInSkiOut ? badgeActiveClass : 'bg-gray-100 text-gray-600'}`}>
        ...
        <a
          ...
          className={`flex-shrink-0 inline-flex items-center gap-1.5 text-sm font-medium ${ctaClass} text-white px-4 py-2 rounded-lg transition-colors no-underline whitespace-nowrap`}
        >
          Check on Hotels.com
