...
import { ParkWeatherMonth } from '@/lib/types'

export default function ParkWeatherChart({ data, destinationName }: { data: ParkWeatherMonth[]; destinationName: string }) {
  const chartData = data.map((m) => ({
    month: m.month.slice(0, 3),
    precipitation: m.avgPrecipitationInches,
    high: m.avgHighF,
    low: m.avgLowF,
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="text-lg font-bold text-gray-900 mb-4">Monthly Precipitation (inches)</h3>
        <div className="bg-white rounded-xl border border-gray-100 p-4">
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={chartData}>
              ...
              />
              <Bar dataKey="precipitation" fill="#60a5fa" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
